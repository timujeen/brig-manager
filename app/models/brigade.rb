class Brigade < ActiveRecord::Base
  ...
  def self.filtered_and_ordered_by_params(country_id, job_ids, order_string)
    ...
    if country_id == 0 && job_ids == []
      ...
      return joins(:jobs).where("brigades_jobs.job_id IN #{jobs_string}").group("brigades_jobs.brigade_id").order(order_query)
    else
      return where(country_id: country_id).joins(:jobs).where("brigades_jobs.job_id IN #{jobs_string}").group("brigades_jobs.brigade_id").order(order_query)
    end
  end
